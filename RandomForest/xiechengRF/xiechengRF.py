...
## 并用其构建随机森林分类模型，指定n_estimators参数为10，
...
for y_test_item in y_test:
    RMSEerrors=RMSEerrors+(y_test_item - y_pred[i])**2
    i=i+1

RMSE=(RMSEerrors/ len(y_test))**(1/2)
print("RMSE:",RMSE)
...
MADerror=0
for y_test_item in y_test:
    MAPEerror=MAPEerror+((y_test_item-y_pred[i])/y_test_item)
    MADerror=MADerror+((y_pred[i]-y_test_item)**2)**(1/2)
    i=i+1

MAPE=MAPEerror/len(y_test)
print("MAPE:",MAPE)

MAD=MADerror/len(y_test)
print("MAD:",MAD)
